...
export default function InvoiceFormModal({
  ...
  >({
    ...
    client: { name: "", address: "", email: "", gstNumber: "" },
    items: [{ description: "", qty: 1, rate: 0 }],
    taxes: [{ name: "GST", percent: 0 }], // ✅ dynamic tax array
    notes: "",
    paymentMethod: "Bank Transfer",
    status: "draft",
  });
  ...
  // 💰 Real-time totals
  const subtotal =
    form.items?.reduce((acc, item) => acc + item.qty * item.rate, 0) || 0;

  const totalTaxPercent =
    form.taxes?.reduce((acc, t) => acc + (t.percent || 0), 0) || 0;
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      ...
      >
        ...
        <section>
          ...
          <div className="mt-3">
            <h4 className="font-medium text-gray-700 mb-2">Bank Details</h4>
            {["bankName", "accountName", "accountNumber", "ifscCode"].map(
              (field) => (
                <input
                  key={field}
                  placeholder={field.replace(/([A-Z])/g, " $1")}
                  className="border p-2 w-full mb-2"
                  value={(form.company.bankDetails as any)[field] || ""}
                  onChange={(e) =>
                    setForm({
                      ...form,
                      company: {
        ...
        <section>
          ...
          {["name", "address", "email", "gstNumber"].map((field) => (
            <input
              key={field}
              placeholder={`Client ${field.replace(/([A-Z])/g, " $1")}`}
              className="border p-2 w-full mb-2"
              value={(form.client as any)[field] || ""}
              onChange={(e) =>
                setForm({
                  ...form,
                  client: { ...form.client, [field]: e.target.value },
                })
              }
            />
          ))}
          ...
          {form.taxes?.map((tax, i) => (
            <div key={i} className="flex gap-2 mb-2">
              ...
              <input
                ...
                value={tax.percent || ""}
                onChange={(e) =>
                  updateTax(i, "percent", Number(e.target.value) || 0)
                }
              />
              <button
                type="button"
